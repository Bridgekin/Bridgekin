class Api::SalesNetworksController < ApiController
  ...
  def index
    ...
    @network_details = SalesNetwork.get_network_info(@current_user)

    render :index
  end

  def search_networks
    @sales_networks = SalesNetwork.includes(:members, :subscriptions, 
      :subscribed_products)
      .where("LOWER(domain) LIKE ?" , "%" + params[:domain].downcase + "%")
